...
dataset_path = "D:\Sem-3\Intro to Python\Project\Augmentation\Datasets\HD-Positive"

# Create an Augmentor Pipeline
pipeline = Augmentor.Pipeline(dataset_path ,output_directory="D:\Sem-3\Intro to Python\Project\Augmentation\Datasets\HD-Positive Aug") #creating an augmentor pipeline (a class in the library) object specifying the input path and output path as parameters
...
pipeline.zoom_random(probability=0.4, percentage_area=0.96)#this line of code adds a random zoom operation to the Augmentor pipeline with a 40% chance of being applied to each image, and the zoomed images will retain 96% of their original area.
